...
3. 执行`docker exec -it star-api-demo bash`进入容器
4. 执行`supervisorctl stop webapp`停止服务，并执行`star run`或`python wsgi.py`进入开发者模式
...
1. 创建`star`网络：`docker network create star`

   可能遇到网络冲突问题，解决方法：
    1. 删除网络：`docker network rm star`
    2. 创建网络时指定掩码和网关：

       `docker network create --subnet=192.168.0.0/24 --gateway=192.168.0.1 star`
